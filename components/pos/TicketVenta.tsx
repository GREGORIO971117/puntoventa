    ...
    return (
        <section className="flex flex-col h-full p-4 lg:p-6">
                        ...
                        <div key={item.id} className="group flex flex-col p-3 bg-white border border-slate-100 rounded-xl shadow-sm hover:border-blue-200 transition-colors">
                            ...
                            <div className="flex justify-between items-center mt-1">
                                ...
                                <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1">
                                    <button onClick={() => onRestar(item.id)} className="w-7 h-7 flex items-center justify-center bg-white rounded shadow-sm text-slate-600 hover:text-blue-600 hover:bg-blue-50 transition-all">
                                        <Minus className="w-4 h-4" />
                                    </button>
                                    <span className="w-8 text-center text-sm font-bold text-slate-700">{item.cantidad}</span>
                                    <button onClick={() => onAgregar(item)} className="w-7 h-7 flex items-center justify-center bg-white rounded shadow-sm text-slate-600 hover:text-blue-600 hover:bg-blue-50 transition-all">
            ...
            <div className="pt-4 mt-2 border-t border-slate-200 bg-white">
                ...
                <Button variant="ghost" className="w-full mt-2 text-slate-400 hover:text-red-600 hover:bg-red-50" onClick={onVaciarCarrito} disabled={carrito.length === 0}>
                    Vaciar Ticket
                </Button>
            </div>
        </section>
